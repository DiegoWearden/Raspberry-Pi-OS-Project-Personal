...
uint32_t RPi_IO_Base_Addr = 0x3F000000;				// RPI IO base address auto-detected by SmartStartxx.S
uint32_t RPi_ARM_TO_GPU_Alias;			// RPI ARM_TO_GPU_Alias auto-detected by SmartStartxx.S
uint32_t RPi_BootAddr;					// RPI address processor booted from auto-detected by SmartStartxx.S
uint32_t RPi_CoresReady;					// RPI cpu cores made read for use by SmartStartxx.S
uint32_t RPi_CPUBootMode;				// RPI cpu mode it was in when it booted
CPU_ID RPi_CpuId;						// RPI CPU type auto-detected by SmartStartxx.S
CODE_TYPE RPi_CompileMode;				// RPI code type that compiler produced
uint32_t RPi_CPUCurrentMode;				// RPI cpu current operation mode
SMARTSTART_VER RPi_SmartStartVer;		// SmartStart version
void* RPi_coreCB_PTR[4];					// RPI pointer to the TCB block for each core .. needs to be set by task system	
